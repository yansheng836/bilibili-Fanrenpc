    ...
    for item in data:
        ...
        content = content + '|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|\n' \
                  % (
                      ...
                      item.get('title', ''),
                      item.get('long_title', ''),
                      format_number_string(stat.get('view', 0)),
                      format_number_string(stat.get('like', 0)),
                      format_number_string(stat.get('coin', 0)),
                      format_number_string(stat.get('favorite', 0)),
                      format_number_string(stat.get('dm', 0)),
                      format_number_string(stat.get('share', 0)),
                      format_number_string(stat.get('reply', 0)))
    # 如果是汇总的，添加汇总行
    if 'TOP10' not in title:
        # print('汇总行')
        content = content + '|汇总|||播放量|点赞数|投币数|收藏数|弹幕数|分享数|评论数|\n'
        # 安全获取 stat 数据，处理 None 的情况
        def safe_sum(key):
            return sum((item.get('stat') or {}).get(key, 0) for item in data)
        content = content + '|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|\n' \
                  % (
                      '汇总', ' ',' ',
                      format_number_string(safe_sum('view')),
                      format_number_string(safe_sum('like')),
                      format_number_string(safe_sum('coin')),
                      format_number_string(safe_sum('favorite')),
                      format_number_string(safe_sum('dm')),
                      format_number_string(safe_sum('share')),
                      format_number_string(safe_sum('reply')))
...
def draw_bar(data, value_type='view', title='这是默认图标名，也是文件名', x_title='集数', y_title='数量'):
    ...
    # 尽量简化x轴的中文，避免互相重叠
    categories = [item["title"] + '.' + (item.get("long_title") or '').replace('凡人风起天南', '风起天南').replace(' ', '') for item in data]
    # values = [230, 450, 560, 780, 320]
    # values = [item["stat"]["view"] for item in data]
    values = [(item.get("stat") or {}).get(value_type, 0) for item in data]
    # 使用色彩映射
